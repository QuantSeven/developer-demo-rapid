		...
		ajaxLoad:function(url,settings,callback){
			...
			var load = $.ajax({
				...
				success: function(response){
					...
					//alert(1111);
					// 判断是否超时
                	if(result.stateCode != null && result.stateCode != "" && result.stateCode != "undefine" && result.stateCode =="301"){
                		//alert(result.stateCode);
                		window.location.href=result.url;// 超时跳转
                		return;
                	}
	...
	Viewui.prototype.ajaxData = function(url, settings,callback) {
		...
		var load = $.ajax({
			...
			success : function(response) {
	            	...
	            	if(result.stateCode  && result.stateCode =="301"){
	               	    window.location.href=result.url;// 超时跳转
	            	}
	            	if(result.statusCode && result.message && result.message != ""){
						viewui.tipAlert(result.statusCode,result.message);
...
; (function ($) {
    ...
    $.widget("viewui.tabs", {
        ...
        _setProperties: function(){
            ...
            tabs.on("dblclick.tabs","li",function(event){
                  var tabAttr = $.data($(this)[0], 'tabs.tab');
                  that._trigger("dblclick",event,tabAttr);
              });
					...
					"viewui.modal",
					{
						options : {
							...
							position : {
								...
								at : "center",
								of : window,
								collision : "fit",// ensure that the titlebar
								// is never outside the
								// document
								using : function(pos) {
						...
						_wrapModal : function() {
							...
							this.uiModal = $("<div/>").addClass("modal")
									.addClass(opts.modalCls).appendTo("body")
									.hide();
							var mheader = $("<div/>").addClass("modal-header")
									.appendTo(this.uiModal), mtitle = $("<h3/>")
									.appendTo(
											$("<div class='float_left title'>")
													.appendTo(mheader)).text(
											opts.title
													|| this.element
															.attr("title")), mbody = $(
									"<div/>").addClass("modal-body").appendTo(
									this.uiModal).append(this.element), mfoot = $(
									"<div>").addClass("modal-footer").appendTo(
									this.uiModal)
							mfoot = opts.buttons.length ? mfoot.show() : mfoot
									.hide();
							...
							var btns = $(
									"<span class='float_right buttons'></span>")
									.appendTo(mheader);

							opts.minBtn
									&& $('<a class="modal_min button"></a>')
											.click(function(event) {
												that.min(event);
											}).appendTo(btns);

							opts.newwinBtn
									&& $("<a class='modal_resize button'></a>")
											.click(function(event) {
												that.newwin(event)
											}).appendTo(btns);

							opts.closeBtn
									&& $("<a class='modal_close button'/>")
											.click(function(event) {
												that.close(event);
											}).appendTo(btns);
							// mheader.prepend('<button type="button"
							// class="titleBar close">&times;</button>');
							// mheader.prepend('<button type="button"
							// class="titleBar close">&times;</button>');
							return this.uiModal;
						},
						...
						_wrap : function() {
							this.viewModal = $("<div/>").addClass("modal")
									.appendTo(modalArea.content);
							var mheader = $("<div/>").addClass("modal-header"), mtitle = $(
									"<h3/>").text(
									this.options.title
											|| this.element.attr("title"));
							mbody = $("<div/>").addClass("modal-body").append(
									this.element);
							mfooter = $("<div/>").addClass("modal-footer");
							this.viewModal.append(header.append(title)).append(
						...
						_size : function() {
							...
							if (options.height === "auto") {
								...
								this.element.height(Math.max(0, Math.min(
						...
						_position : function(position) {
							...
							if (position) {
								...
								position = $
										.extend(
												{},
												$.viewui.modal.prototype.options.position,
												position);
							} else {
						...
						_makeResizable : function(handles) {
							...
							// positioning
							position = this.uiModal.css("position"), resizeHandles = typeof handles === 'string'
									? handles
									: "n,e,s,w,se,sw,ne,nw";
						...
						open : function() {
							...
							if (opts.singleSelect) {
								opts.bgdesk = $('<div class="modal-backdrop fade in"></div>').css("z-index",$.viewui.zIndex++)
										.appendTo("body");
							}
						...
						loadRemote : function(url) {
							var opts = this.options, that = this;
							this.element.ajaxLoad(url, {
								data : opts.requestParam
							}, function(response) {
						...
						title : function(title) {
							this.options.title = title
									|| this.element.attr("title")
							this.uiModal.find(".title h3").html(
						...
						newwin : function(event) {
							...
							if (win.hasClass('window_full')) {
								this._size();
								this.moveToTop();
								win.removeClass("window_full").draggable(
										"enable");
								this.element.find(">div").triggerHandler(
										"_resize");
							} else {
								this.element
										.height($(window).height()
												- (win.height() - this.element
														.height()));
								win.width($(window).width()).addClass(
										'window_full').position({
									my : "center",
									at : "center",
									of : window
								}).draggable("disable").removeClass(
										"ui-state-disabled");
								this.element.find(">div").triggerHandler(
										"_resize");
						...
						addButton : function(name, prop, cls) {
							var opts = this.options, that = this, btn, mfoot = this.uiModal
						...
						close : function(event) {
							...
							this.options.bgdesk && this.options.bgdesk.remove();
							this.uiModal.effect("clip", function() {
								$(this).remove()
							});
						...
						moveToTop : function(event) {
							...
							if (opts.zIndex > $.viewui.zIndex) {
								$.viewui.zIndex = opts.zIndex;
							}

							this.uiModal.css("z-index", $.viewui.zIndex++);
    ...
    $.widget("viewui.datagrid", {
        ...
        */
        _loadData: function () {
		    ...
		    $.each(this.selectedRows||[],function(i,v){
		    	
		    	$.each(opts.data.dataList,function(k,f){
		    		if(JSON.stringify(v)===JSON.stringify(f)){
		    			var tr = that.grid.find("tr[datagrid-row-index='"+k+"']");
		    			tr.length>0 && that.selectRow(that.element,tr[0]);
        ...
        _request: function (target) {
            ...
            $.ajax({
                ...
                success: function (data) {
                	...
                	// 判断是否超时
                	if(data.stateCode != null && data.stateCode != "" && data.stateCode != "undefine" && data.stateCode =="301") {
                		window.location.href=data.url;// 超时跳转
                		return;
                	}
        ...
        */
        unselectRow: function (target, tr) {
            ...
            var index =  $.inArray(opts.data.dataList[$(tr).attr('datagrid-row-index')],this.selectedRows);
        ...
        selectRow:function (target, tr) {
            ...
            if (opts.singleSelect == true) {
            	this.unselectRow(null,$(tr).siblings().removeClass('datagrid-row-selected').find("input[type=checkbox].datagrid-cell-check").attr('checked', false));
            	this.selectedRows=[];
            	this.selectedRows.push(data.dataList[$(tr).attr('datagrid-row-index')])
            }else{
            	 var row = data.dataList[$(tr).attr('datagrid-row-index')];
                 $.inArray(row,this.selectedRows)==-1&&this.selectedRows.push(row);
            }
        ...
        getSelectedRows: function (target, idkey) {
            ...
            var rows = [];
               ...
               $.each(this.selectedRows,function(i,v){
	            	if(v&&($.inArray(v,opts.data.dataList)>-1) &&idkey){
	            		rows.push(v[idkey]);
	            	}else if(v&&($.inArray(v,opts.data.dataList)>-1)){
	            		rows.push(v);
	            	}
              });
            return rows;
